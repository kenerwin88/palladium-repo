...
SBOM checksum, deterministic migration-bundle checksum, migration-set hash, production schema-plan
and deployment-receipt hashes, Terraform state keys, exact production plan artifact name, and
verified environment URLs are the evidence. A legacy pinned environment accepts only a release whose
manifest digest and migration bundle equal the permanent published release evidence.
...
`release-manifest-delivery-<run-id>-a<attempt>.json`; it never overwrites audit evidence. Legacy
pinned environments use the canonical manifest while the appended files preserve the full execution
history.
